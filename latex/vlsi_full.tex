...
\begin{frame}
  ...
  \only<1-6>{\visible<2-6>{
	\begin{itemize}[<+(1)->]
	  ...
	  \item ``Κοινή λογική''
	  \item[] Και αυτά με τρόπο \em{unsupervised}, πραγματοποιώντας διαρκώς \em{προβλέψεις}.
	\end{itemize}
  }}
...
\begin{frame}{Sequence memory}
  \only<1>{ \includegraphics[width=1\textwidth]{../pics/sequence-learning-big} }
  \only<2>{ \includegraphics[width=1.07\textwidth]{../pics/sequence2} }
\end{frame}
...
Computing Platform \newline
	...
	\begin{frame}
	  \centering
	  \vspace{3em}
	  %\Huge{\textbf{ΕΥΧΑΡΙΣΤΟΥΜΕ ΓΙΑ ΤΗΝ ΠΡΟΣΟΧΗ ΣΑΣ}}\\
